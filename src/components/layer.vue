    ...
    <template v-if="list.length">
      ...
      <div class="flex flex-col">
        <div
          ...
          :key="item.id"
          :class="isSelect(item) && 'bg-cyan-200'"
          class="bg-slate-100 my-1"
        >
          <div class="flex flex-row">
            <div class="w-3/5 flex flex-row content-center">
              <span
                :class="isSelect(item) && 'active'"
                :v-tooltip="item.name || item.text || item.type"
                v-html="iconType(item.type)"
                class="self-center"
              ></span>
              <span class="self-center">| {{ textType(item.type, item) }}</span>
            </div>
            <div class="w-2/5">
              <Button
                long
                :icon="item.isLock ? 'pi pi-lock' : 'pi pi-unlock'"
                text
                @click="doLock(item)"
              ></Button>
              <Button
      ...
      <!-- 层级调整按钮 -->
      <div class="btn-box flex flex-row mt-4">
        <Button @click="up" text
          ><span v-html="btnIconType('up')"></span
        ></Button>
        <Button @click="down" text
          ><span v-html="btnIconType('down')"></span
        ></Button>
        <Button @click="upTop" text
          ><span v-html="btnIconType('upTop')"></span
        ></Button>
        <Button @click="downTop" text
